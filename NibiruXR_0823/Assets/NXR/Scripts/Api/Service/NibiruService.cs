...
namespace Nxr.Internal
{
    ...
    public class NibiruService
    {
        ...
        public void Init()
        {
            ...
            nibiruSensorServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getNibiruSensorService", androidActivity);
            nibiruVoiceServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getNibiruVoiceService", androidActivity);
            nibiruGestureServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getNibiruGestureService", androidActivity);
            nibiruVRServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getUsingNibiruVRServiceGL");

            nibiruCameraServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getNibiruCameraService", androidActivity);
            nibiruMarkerServiceObject = nibiruSDKClass.CallStatic<AndroidJavaObject>("getNibiruMarkerService", androidActivity);
        ...
        public void RegisterVirtualMouseService(OnVirtualMouseServiceStatus serviceStatus)
        {
            ...
            {
                nibiruOsServiceObject.Call("registerVirtualMouseManagerService", new NibiruVirtualMouseServiceListener(serviceStatus));
            }
            else
            {
        ...
        /// <summary>
        ///  控制手势服务相机状态：false关闭相机占用，true开启相机占用
        /// </summary>
        /// <param name="enabled"></param>
        ...
        /// <summary>
        /// sensor data changed callback
        /// </summary>
        public static OnSensorDataChanged OnSensorDataChangedHandler;

        class NibiruSensorDataListenerCallback : AndroidJavaProxy
        {
            public NibiruSensorDataListenerCallback() : base("com.nibiru.service.NibiruSensorService$INibiruSensorDataListener")
            { }
        ...
        public void RegisterSensorListener(SENSOR_TYPE type, SENSOR_LOCATION location)
        {
            if (nibiruSensorServiceObject != null)
            {
                ...
                RunOnUIThread(androidActivity, new AndroidJavaRunnable(() =>
                {
                    AndroidJavaClass locationClass = BaseAndroidDevice.GetClass("com.nibiru.service.NibiruSensorService$SENSOR_LOCATION");
                    AndroidJavaObject locationObj = locationClass.CallStatic<AndroidJavaObject>("valueOf", location.ToString());

                    AndroidJavaClass typeClass = BaseAndroidDevice.GetClass("com.nibiru.service.NibiruSensorService$SENSOR_TYPE");
                    AndroidJavaObject typeObj = typeClass.CallStatic<AndroidJavaObject>("valueOf", type.ToString());

                    nibiruSensorServiceObject.Call<bool>("registerSensorListener", typeObj, locationObj, nibiruSensorDataListenerCallback);
        ...
        {
            IsCaptureEnabled = 1;
            nibiruSDKClass.CallStatic("startCaptureForUnity", new CaptureCallback(), path, bitRate, seconds, videoSize, HMDCameraId);
        }
        ...
        public class NibiruVirtualMouseServiceListener : AndroidJavaProxy
        {
            ...
            public NibiruVirtualMouseServiceListener(OnVirtualMouseServiceStatus onVirtualMouseServiceStatus) : base("com.nibiru.service.NibiruVirtualMouseManager$VirtualMouseServiceListener")
            {
        ...
        /// <summary>
        /// Requset permission,
        /// names refer to NxrGlobal.Permission
        /// </summary>
